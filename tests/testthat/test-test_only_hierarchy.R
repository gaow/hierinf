...
test_that("test_only_hierarchy: check input", {
  ...
  expect_error(test_only_hierarchy(x = NULL, y = matrix(1:4, ncol = 2), dendr = NULL,
                                   res.multisplit = NULL, family = NULL),
  ...
  expect_error(test_only_hierarchy(x = NULL, y = list(matrix(1:4, ncol = 2), matrix(1:4, ncol = 2)), dendr = NULL,
                                   res.multisplit = NULL, family = NULL),
  ...
  expect_error(test_only_hierarchy(x = matrix(1:4, ncol = 2), y = 1:2, dendr = NULL,
                              res.multisplit = NULL, family = NULL),
  ...
  expected_result_1 <- data.frame(block = NA, p.value = NA, significant.cluster = NA)
  expected_result_1$significant.cluster <- list(NA)
  attr(expected_result_1, "class") <- c("data.frame")
  expected_result <- list(res.multisplit = res.multisplit, res.hierarchy = expected_result_1)
  ...
  expect_error(test_only_hierarchy(x = list(sim.geno1, sim.geno2), y = list(y1, y2),
                                   dendr = dendr, res.multisplit = res.multisplit,
                                   family = "gaussian"),
...
check_test_hierarchy <- function(x, y, clvar, res.multisplit, B, cluster_test, binomial = FALSE){
  ...
  for (i in cluster_test) { # for each cluster
    for (b in 1:B) { # for each split (multi-sample splitting)
      ...
      if (ncol(clvar_x_reduced) == 0) {clvar_x_reduced <- rep(1, length(y[ind]))}
...
test_that("test_only_hierarchy: check output (Example I)", {
  expect_equal(res$res.hierarchy$p.value[1], expected_result$p.value[1], tol = 1e-115)
  expect_equal(res$res.hierarchy$p.value[2], expected_result$p.value[2], tol = 1e-85)
  expect_equal(res$res.hierarchy$significant.cluster, expected_result$significant.cluster)
})
...
# multisplit
set.seed(555)
res.multisplit <- multisplit(x = sim.geno, y = y, clvar = sim.clvar, family = "gaussian",
                             B = B)
...
gen_one <- function(n, p, seed1, seed2, seed3, num_clvar = NULL, coef_clvar = NULL) {
  set.seed(seed1)
...
res1 <- check_test_hierarchy(x = x[[1]], y = y[[1]], clvar = NULL, res.multisplit = res.multisplit[1],
                             B = B, cluster_test = cluster_test)

res2 <- check_test_hierarchy(x = x[[2]], y = y[[2]], clvar = NULL, res.multisplit = res.multisplit[2],
                             B = B, cluster_test = cluster_test)

res3 <- check_test_hierarchy(x = x[[3]], y = y[[3]], clvar = NULL, res.multisplit = res.multisplit[3],
                             B = B, cluster_test = cluster_test)

res4 <- check_test_hierarchy(x = x[[4]], y = y[[4]], clvar = NULL, res.multisplit = res.multisplit[4],
                             B = B, cluster_test = cluster_test)
...
test_that("test_only_hierarchy: check output (Example III multiple data sets)", {
  expect_equal(res$res.hierarchy$p.value, expected_result$p.value, tol = 1e-135)
  expect_equal(res$res.hierarchy$significant.cluster, expected_result$significant.cluster)
})
...
test_that("test_only_hierarchy: check output (Example IV multiple data sets)", {
  # expect_equal(res$p.value, expected_result$p.value, tol = 1e-180)
  expect_identical(res$res.hierarchy$p.value, expected_result$p.value)
  expect_equal(res$res.hierarchy$significant.cluster, expected_result$significant.cluster)
})
...
res1 <- check_test_hierarchy(x = x[[1]], y = y[[1]], clvar = clvar[[1]], res.multisplit = res.multisplit[1],
                             B = B, cluster_test = cluster_test)

res2 <- check_test_hierarchy(x = x[[2]], y = y[[2]], clvar = clvar[[2]], res.multisplit = res.multisplit[2],
                             B = B, cluster_test = cluster_test)

res3 <- check_test_hierarchy(x = x[[3]], y = y[[3]], clvar = clvar[[3]], res.multisplit = res.multisplit[3],
                             B = B, cluster_test = cluster_test)

res4 <- check_test_hierarchy(x = x[[4]], y = y[[4]], clvar = clvar[[4]], res.multisplit = res.multisplit[4],
                             B = B, cluster_test = cluster_test)
...
test_that("test_only_hierarchy: check output (Example VI multiple data sets)", {
  expect_identical(res$res.hierarchy$p.value, expected_result$p.value)
  expect_equal(res$res.hierarchy$significant.cluster, expected_result$significant.cluster)
})
...
test_that("test_only_hierarchy: check output (Example with smaller tree)", {
  expect_equal(res$res.hierarchy$p.value, expected_result$p.value, tol = 1e-120)
  expect_equal(res$res.hierarchy$significant.cluster, expected_result$significant.cluster)
})
...
test_that("test_only_hierarchy: check output (Example with smaller tree; no global)", {
  expect_equal(res$res.hierarchy$p.value, expected_result$p.value, tol = 1e-120)
  expect_equal(res$res.hierarchy$significant.cluster, expected_result$significant.cluster)
})
...
test_that("test_only_hierarchy: check return object for multiple data sets not measuring the same variables", {
  ...
  res1 <- check_test_hierarchy(x = tt1, y = y1, clvar = NULL, res.multisplit = res.multisplit[1],
                               B = B, cluster_test = cluster_test)

  res2 <- check_test_hierarchy(x = tt2, y = y2, clvar = NULL, res.multisplit = res.multisplit[2],
                               B = B, cluster_test = cluster_test)

  res3 <- check_test_hierarchy(x = tt3, y = y3, clvar = NULL, res.multisplit = res.multisplit[3],
                               B = B, cluster_test = cluster_test)
...
test_that("test_only_hierarchy: check return object for a data set with binary response", {
  ...
  expect_equal(res$res.hierarchy$p.value[1], expected_result$p.value[1], tol = 1e-25)
  expect_equal(res$res.hierarchy$p.value[2], expected_result$p.value[2], tol = 1e-20)
  expect_equal(res$res.hierarchy$significant.cluster, expected_result$significant.cluster)
})
...
test_that("test_only_hierarchy: check return object for two data set with binary response", {
  ...
  expect_equal(res$res.hierarchy$p.value[1], expected_result$p.value[1], tol = 1e-40)
  expect_equal(res$res.hierarchy$p.value[2], expected_result$p.value[2], tol = 1e-30)
  expect_equal(res$res.hierarchy$significant.cluster, expected_result$significant.cluster)
})

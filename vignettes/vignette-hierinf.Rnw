...
@
The function \texttt{compute\char`_r2} calculates the adjusted $\mbox{R}^2$ value or
coefficient of determination of a cluster for a continuous response. The
Nagelkerke's $\mbox{R}^2$ \citep{nagelkerke91} is calculated for a binary
response as e.g. in our toy
example.
